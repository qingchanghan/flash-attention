...
template <typename T>
void print_to_file_with_blank(const T* result, const int size, const char* file,
                              cudaStream_t stream = 0,
                              std::ios::openmode open_mode = std::ios::out);

template <typename T>
void print_to_file_with_batch_decompose(
    const T* result, const int size, const char* file, const int batch_size,
    cudaStream_t stream, std::ios::openmode open_mode = std::ios::out);
...
void print_to_screen_int4(const int8_t* result, const int size);

template <typename T>
void print_to_kv_screen(const T** result, const int size);

template <typename T>
void print_kv_cache_value(const T** result, const int cur_batch,
                          const int size);
...
void compareTwoTensor(const T1* pred, const T2* ref, const int size,
  ...
  for (int i = 0; i < size; i++) {
    if (i < print_size) {
      FT_LOG_INFO("%4d | % 6.4f | % 6.4f | % 6.4f | % 7.4f |", i,
                  (float)h_pred[i], (float)h_ref[i],
